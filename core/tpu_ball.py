# (__module__/snippet) <transformer_ball> 
"""transformer_ball
__version__: "2022.12.20"
<model: chatGPT by OpenAI /Generative Pre-trained Transformer/>
<tpu_ball.py is __module__ of lottGPT //__spec__ of tpu.py>
...
def load_file(key):
    ...
    df = pd.read_csv(file)
    # print(f"== <tpu_ball.load_file> \n>> df_in \n{df}")

    # ball/mega/power/keno
    ## suffix ## del_[n,k,b7]
    if key == "mega" : df = df.drop(columns=["n","k"])
    if key == "power": df = df.drop(columns=["n","k","b7"])
    ## to_list /[list-2d]/
    ls = df.values.tolist()
